...
export const Column = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      sortable: true,
      sortField: "client.name",
      cell: (row, index) => (index % 2 === 0 ? "Офтальмология" : "Терапия"),
    },
    {
      name: t("Position"),
      sortable: true,
      sortField: "client.name",
      cell: (row, index) => (index % 2 === 0 ? "Офтальмолог" : "Терапевт"),
    },
    {
      name: t("Phone"),
      sortable: true,
    ...
    },
    {
      ...
      cell: (row) => (
        <div className="column-action d-flex gap-1 align-items-center">
          <Link id={`pw-tooltip-${row.id}`} to={`/personal/preview/${row.id}`}>
            <Eye size={17} />
          </Link>
          <Link id={`up-tooltip-${row.id}`} to={`/personal/edit/${row.id}`}>
            <Edit size={17} />
          </Link>
          <a
